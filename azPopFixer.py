import IEDeformers
fnVR = IEDeformers.FnVertexRemap('ieVertexRemap1')
fnVR.setReference('ReferenceShape')
cd.ieCage(deformedGeo=inGeo, side=side, description=inGeo.replace((side+'_'), ''),
driverGeo=driverGeo, parent=self.miscGrp)



import maya.cmds as mc, maya.mel as mm

def pop_fixer(face_selection=None):
	...
	##
	# point 2 point
	cDeformer = RigToolbox.Core.DeformerUtils.CreateDeformer()
	cDeformer.ieVertexRemap(deformedGeo='pSphere1', driverGeo='pCube1', loadMap=False)

	deformer = cDeformer.deformer
