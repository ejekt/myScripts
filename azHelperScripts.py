#import CfxToolbox
#nio = CfxToolbox.Core.NodeAttributesIO.NodeAttributesIO()
#nio.readAll()
...
def toggleOutput():
	ctrl = '*:c_master_CTRL'
	if mc.getAttr(ctrl+'.cfxOutput'):
		mc.setAttr(ctrl+'.cfxOutput', 0)
		mc.setAttr(ctrl+'.cfxDeform', 1)
	else:
		mc.setAttr(ctrl+'.cfxOutput', 1)
		mc.setAttr(ctrl+'.cfxDeform', 0)
...
def createFollowCam(constrainTo=None):
	...
	sCamera = mc.camera(n='followCam', centerOfInterest=5, focalLength=35, lensSqueezeRatio=True, cameraScale=True,
						horizontalFilmAperture=0.967717, horizontalFilmOffset=False, verticalFilmAperture=0.735238,
						verticalFilmOffset=False, filmFit='Fill', overscan=True, motionBlur=False, shutterAngle=144,
						nearClipPlane=0.1, farClipPlane=10000, orthographic=False, orthographicWidth=30,
						panZoomEnabled=False, horizontalPan=False, verticalPan=False, zoom=True)[0]
...
def create_shaders(colours=['red', 'blue', 'yellow']):
	for color in colours:
		...
		if color == 'white':
			mc.setAttr(shader + '.color', *[1, 1, 1])
	# to assign
	# mc.sets('geos', e=1, fe='color_SG')
...
def createIk(name='L_hip', solver='ikRPsolver', startJoint='', endEffector='', poleVector='', parent='', ctrl='',
			 consType='parent', v=1):
	'''
	Create ikRP solver.
...
def addSpaceSwitch(ctrl, attrName, enums, slave, targets, constraintType, maintainOffset):
	...
	if cmds.objExists(ctrl + '.' + attrName):
		cmds.warning(
			'"' + ctrl + '.' + attrName + '" exist, using the exitant one, check if attribute match you\'re expecting')
...
def createBlendShape_between_SIMandOUT():
	bsList = []
	geos = cmds.ls(sl=True)
	for geo in geos:
		driver = geo.replace('_OUT', '_SIM')

		bs = cmds.blendShape(driver, geo, origin='world', n=geo + '_BS')
		cmds.setAttr(bs[0] + '.' + driver, 1)
		bsList.append(bs)
	return bsList

createBlendShape_between_SIMandOUT()


def connectOutMeshInMesh_between_SIMandOUT():
	geos = cmds.ls(sl=True)
	for geo in geos:
		driver = geo.replace('_OUT', '_SIM')
		driverShape = maya.cmds.listRelatives(driver, shapes=True)[0]
		geoShape = maya.cmds.listRelatives(geo, shapes=True)[0]
		bs = createBlendShape_between_SIMandOUT()
		cmds.delete(ch=True)
		cmds.connectAttr(driverShape+'.outMesh', geoShape+'.inMesh', f=True)

connectOutMeshInMesh_between_SIMandOUT()


def connect_between_nClothAndOUT():
	geos = cmds.ls(sl=True)
	for geo in geos:
		geoShape = maya.cmds.listRelatives(geo, shapes=True)[0]
		driver = cmds.listConnections(geoShape, t='nCloth')[0]
		driverShape = maya.cmds.listRelatives(driver, shapes=True)[0]
		outShape = geoShape.replace('SIM','OUT')

		cmds.connectAttr(driverShape+'.outputMesh', geoShape+'.inMesh', f=True)

connect_between_nClothAndOUT()



##
# Maia's edge face selection automagic

class edgeFaceSelection():
	def __init__(self):
		print 'Edge face selection tool'

	def part1(self):
		# Find object from selection
		sel = cmds.ls(sl=True)

		obj = sel[0].partition('.')[0]
		faceNum = (cmds.polyEvaluate(obj, f=True) - 1)

		# Find CH before creation
		origHist = cmds.listHistory(obj)

		# Project UVs onto new set and cut along selected edges
		set = cmds.polyUVSet(create=True, uvSet='tempUVSet')
		cmds.polyUVSet(currentUVSet=True, uvSet=set[0])
		cmds.polyProjection(obj + '.f[0:%i]' % faceNum, t='Planar')
		cmds.polyMapCut(sel)

		# Find CH after and compare to original
		self.toDelete = []
		newHist = cmds.listHistory(obj)
		for h in newHist:
			if h not in origHist:
				self.toDelete.append(str(h))

		# Enter into selection by UV shell context
	...
	def part2(self):
		cmds.delete(self.toDelete)


# efs = edgeFaceSelection()
#
# efs.part1()
# qqwq
...
# Maia's utils
def createCrvFromEdges():
	edges = cmds.ls(sl=True)
	obj = edges[0].partition('.')[0]

	crv = cmds.polyToCurve(n=obj + '_CRV')[0]

	cmds.FreezeTransformations(crv)
	cmds.DeleteHistory(crv)
